  ...
  async execute(interaction: CommandInteraction, client: ClientBase) {
      ...
      try {
        if ((interaction.member as GuildMember).roles.cache.get(role.id)) {
          ...
          await interaction.reply({
            content: `You have been removed from the ${role} role.`,
            ephemeral: true,
          });
        } else {
          await (interaction.member as GuildMember).roles.add(role.id);
          await interaction.reply({
            content: `You have been added to the ${role} role.`,
            ephemeral: true,
          });
        }
      } catch (err) {
        await interaction.reply({
          content:
            "Something went wrong while attempting to execute this interaction.",
          ephemeral: true,
        });
        console.error(err);
      }
    ...
    if (user.cooldowns[command.name] < Date.now()) {
      await UserModel.findOneAndUpdate(
        ...
        delete user.cooldowns[command.name] as unknown as UpdateQuery<UserType>
      );
    }
    ...
    if (user.cooldowns[command.name] > Date.now()) {
      const timeleft: string = prettyms(
        user.cooldowns[command.name] - Date.now()
      );

      return contentTipMessage(
        interaction,
        `You are on cooldown! You still have to wait \`${timeleft}\`.`,
        true
      );
    }
    ...
    if (command.ownerOnly && !client.owners.includes(interaction.user.id))
      return contentTipMessage(
        interaction,
        "This action is restricted to the bot developers only.",
        true
      );

    if (
      command.botModeratorOnly &&
      !(
        client.botModerators.includes(interaction.user.id) ||
        client.owners.includes(interaction.user.id)
      )
      ...
      return contentTipMessage(
        interaction,
        "This action is restricted to the bot moderators only.",
        true
      );
    ...
    try {
      ...
      console.error(error);
      await interaction.reply({
        content: `There was an unexpected error while attempting to execute this interaction.\n\`${error.message}\``,
        ephemeral: true,
      });
    }
  },
