...
export = <Command>{
  name: "roles",
  description:
    "Custom action created by Hauntless#3212 for [Spy Dankers](https://discord.gg/thespy).",
  async execute(interaction: CommandInteraction) {
    const guildId = spyDankersGuildId;

    if (interaction.guild.id !== guildId) return;

    if (!(interaction.member as GuildMember).roles.cache.has(spyDankersRoleId))
      return;
    ...
    await interaction.reply({
      content: `${interaction.guild.name}'s Self Roles`,
