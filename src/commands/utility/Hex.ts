...
import { MessageAttachment, ButtonInteraction, CommandInteraction, Message, MessageActionRow, MessageButton } from "discord.js";
...
export = <Command> {
	...
	async execute(interaction: CommandInteraction, client: ClientBase) {
		const match: RegExpMatchArray = interaction.options.getString("hex", true).match(/#?[a-fA-F0-9]{6}/);

		if (!match) {return contentTipMessage(interaction, "An invalid hex code was provided!", true);};

		const query = match[0];
		const hex = query.startsWith("#") ? query.toUpperCase() : "#" + query.toUpperCase();
		const color = getcolor(hex);

		if (color[1].startsWith("Invalid Color")) {return contentTipMessage(interaction, "Something went wrong while parsing your hex code.", true);};

		const buf: Buffer = await sharp(Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 127.14 96.36" height="128" width="128">
            <style>
                .cls-1 { fill: ${hex}; }
            </style>
		...
		await interaction.reply({ content: `\`${color[1]}\` ~ **${hex}**`, files: [file], components: [ButtonRow] });
