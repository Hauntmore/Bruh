...
export = <Command> {
	...
	async execute(interaction: CommandInteraction, client: ClientBase) {
        ...
        const command = commands.find((c) => c.name?.toLowerCase() === args?.toLowerCase());
		...
		if (!args) {
            ...
            for (const folder of folders) {
                const files = readdirSync(join(__dirname, "../", folder)).filter((file) => file.endsWith(".js") && file.startsWith("index"));
                ...
                for (const file of files) {
                    ...
                    categories.push(`\`${category.default.name}\`\n[\`/help ${category.default.label}\`](${process.env.DISCORD_BOT_INVITE})`);
                }
            }

            const map = categories.join("\n\n");

            embed.setDescription("Here is a list of the available categories!\n\n" + map);
        } else if (command) {
            ...
            if (command.usage) {
				embed.addField("**Usage**:", Array.isArray(command.usage)
					? `${command.usage.map((u: string) => `\`\`\`css\n${u}\n\`\`\``).join("\n")}`
					: `\`\`\`css\n${command.usage}\n\`\`\``,
				);
			}

            if (command.cooldown) {embed.addField("**Cooldown**:", `\`\`\`js\n${command.cooldown}s\n\`\`\``, false);};
        ...
        } else if (set.includes("owner") && !(client.botModerators.includes(interaction.user.id) || client.owners.includes(interaction.user.id))) {
            return;
        } else {
            return contentTipMessage(interaction, "Why is it that hard for you to give me a query that exists.", true);
        }
