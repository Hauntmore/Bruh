...
export = <Command> {
	...
	async execute(interaction: CommandInteraction, client: ClientBase) {
        ...
        if (command === "pepe") {
            ...
            const bg = await Canvas.loadImage("https://github.com/katie07/Imagayes/blob/main/oo.png?raw=true");
            ...
            if (user.id === client.user.id) return contentTipMessage(interaction, "You wish lmao.", false);

            const buffer = await (await fetch("https://api.monkedev.com/canvas/gay?imgUrl=" + encodeURIComponent(user.displayAvatarURL({ format: "jpg" })))).buffer();
            ...
            if (text.length >= 71) return contentTipMessage(interaction, "Please input text that is under 71 characters.", true);

            const res = await (await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=clyde&text=${text}`))).json();
