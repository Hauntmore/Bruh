  ...
  options: [
    {
      ...
      type: 4,
      description:
        "The amount of messages to delete (defaults 15 if no value is given).",
      required: false,
    },
  ],
  async execute(interaction: CommandInteraction) {
    ...
    if (
      ...
      return contentTipMessage(
        interaction,
        "You need the `MANAGE_MESSAGES` permission to execute this action!",
        true
      );
    ...
    try {
      ...
      const finalResult = `${deletedMessages.size} message${
        deletedMessages.size > 1 ? "s were pruned." : " was pruned!"
      }\n\n${userMessageMap
