	...
	async execute(interaction: CommandInteraction, client: ClientBase) {
        try {
			...
			if (type === MessageEmbed || type === MessageAttachment) {
                return await interaction.reply({ content: `Time: \`${time.toFixed(3)} ms\`\n${evaled}` });
			...
			if (evaled.length > 1900) {
                ...
                await interaction.reply({ content: `${formatted}, \`\`\`ts\n${evaled}\`\`\`` });
            }
		} catch (err) {
			...
			if (err.length > 1900) {
				...
				await interaction.editReply({ content: "**An error has occured:**", files: [result] });
			} else {
