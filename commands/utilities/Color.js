...
module.exports = {
	name: 'color',
	aliases: ['viewcolor', 'colorinfo', 'hex', 'hexcolor'],
	description: 'View a hex code color.',
	usage: '<hex>',
	example: ['hex #FF0000', 'color AFF2F2'],
	botPermissions: ['EMBED_LINKS', 'SEND_MESSAGES', 'VIEW_CHANNEL', 'READ_MESSAGE_HISTORY'],
	args: true,
	cooldown: 5,
	async execute(message, { args }) {
		const match = args[0].match(/#?[a-fA-F0-9]{6}/);
		if (!match) {return message.reply({ content: 'An invalid hex code was provided!' });}

		const query = match[0];
		const hex = query.startsWith('#') ? query.toUpperCase() : '#' + query.toUpperCase();
		const color = getcolor(hex);

		if (color[1].startsWith('Invalid Color')) {return message.reply({ content: 'Something went wrong while parsing your hex code.' });}
