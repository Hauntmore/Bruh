module.exports = {
	...
	async execute(message, { args }) {
		...
		if (args[0].toLowerCase() === 'message' && args[1].toLowerCase() === 'delete') {
			await client.db.guildWelcomeMessage(message.guild.id, 'null');
			...
			await client.db.guildWelcomeMessage(message.guild.id, content);
		...
		if (args[0].toLowerCase() === 'channel' && args[1].toLowerCase() === 'delete') {
			await client.db.guildWelcomeChannel(message.guild.id, 'null');
			...
			await client.db.guildWelcomeChannel(message.guild.id, channel.id);
